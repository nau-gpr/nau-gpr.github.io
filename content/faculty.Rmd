...
```{r faculty-summary-stats}
# Create faculty summary statistics table
faculty_summary <- data.frame(
  Metric = c(
    "Total Publications",
    "Total Citations", 
    "Department H-Index",
    "Actively Publishing Faculty (past 2 years)"
  ),
  Value = c(
    format(nrow(faculty_data), big.mark = ","),
    format(sum(faculty_data$Citations, na.rm = TRUE), big.mark = ","),
    stats$h_index,
    stats$actively_publishing_count
  )
)
...
knitr::kable(
  faculty_summary,
  format = "html",
  caption = "Faculty Research Summary Statistics",
